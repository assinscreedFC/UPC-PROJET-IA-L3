"""
Tournoi complet entre IA utilisant l'algorithme Minimax avec élagage Alpha-Bêta.
Profondeurs 1 à 4, heuristiques simple/advanced/expert.
Résultats et analyse sauvegardés dans data/elagage/
"""
...
from src.ia.elagage import QuoridorIA
...
# ============================================================
# PHASE ÉLIMINATOIRE (3 poules → top 2 + 2 meilleurs 3e)
# ============================================================
...
def analyze_results(all_results: List[Dict], final_ranking: List[str]):
    """Génère les statistiques et graphiques dans data/elagage/"""
    import pandas as pd
    ...
    print(f"\n{'='*60}")
    print(f"  ANALYSE — Minimax avec élagage Alpha-Bêta")
    ...
    ax.text(6, 13.3, "TOURNOI ALPHA-BÊTA", fontsize=24, fontweight="bold",
            ha="center", color="white", family="monospace")
    ax.text(6, 12.8, f"{len(df)} matchs | {len(all_ias)} participants | Avec élagage",
    ...
    ax.set_title("Performance globale — Minimax + Alpha-Bêta", fontsize=14, fontweight="bold")
    ...
    ax.set_title("Taux de victoire par profondeur — Alpha-Bêta", fontsize=14, fontweight="bold")
    ...
    ax.set_ylabel("Taux de victoire (%)", fontsize=12)
    ax.set_title("Taux de victoire par heuristique — Alpha-Bêta", fontsize=14, fontweight="bold")
    ax.set_ylim(0, 100)
    plt.tight_layout()
    ...
    ax.set_title("Temps de calcul par configuration — Alpha-Bêta",
    ...
    ax.set_title("Longueur des matchs — Alpha-Bêta", fontsize=14, fontweight="bold")
    ...
    ax.set_title("Utilisation des murs par IA — Alpha-Bêta", fontsize=14, fontweight="bold")
    plt.xticks(rotation=30)
    plt.tight_layout()
...
def run_tournament():
    """Lance le tournoi Alpha-Bêta complet : 3 poules + phase éliminatoire."""

    tournament_start = time.time()

    print("\n" + "#" * 60)
    print("#" + " " * 8 + "TOURNOI MINIMAX + ÉLAGAGE ALPHA-BÊTA" + " " * 7 + "#")
    print("#" + " " * 8 + f"12 participants — D1 à D4" + " " * 18 + "#")
    print("#" * 60)
    ...
    filename = f"{OUTPUT_DIR}/results/tournoi_elagage.csv"
